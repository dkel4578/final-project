...
async function checkEmail(email,loginId){
  ...
  return await axios
  .get("/api//user/checkLoginIdByEmail?loginId="+loginId+"&email="+email,{
    "email" : email
    , "loginId" : loginId
  })
  .then((response)=>{
...
const findId= async (email) =>{// eslint-disable-line no-unused-vars
  // console.log("email: ",email);
...
function PasswordChangePage() {
  ...
  const handleSendEmail = async (e) =>{// eslint-disable-line no-unused-vars
    e.preventDefault();
    const response = await  checkEmail(email,loginId);
    console.log("response", response);
    if(!response.isIdCorrect){
      $(".after-send-email").css("color", "red").html(loginId + "은<br>존재하지 않는 아이디입니다.");
    }else if(!response.isEmailCorrect){
      $(".after-send-email").css("color", "red").html(email + "은<br>일치하지 않는 메일입니다.");
    }else{
  ...
  const handleCheckCode = async (e) =>{ // eslint-disable-line no-unused-vars
    e.preventDefault();
